  ...
  <div class="card mb-4 mt-3">
    ...
    <div class="card-body">
      ...
      <form class="geniusform" action="{{route('admin.ps.update')}}" method="POST" enctype="multipart/form-data">
          ...
          <div class="form-group">
            <label for="inp-name">{{  __('Button Name')  }}</label>
            <input type="text" class="form-control" id="inp-name" name="call_button"  placeholder="{{ __('Enter Name') }}" value="{{ $data->call_button }}" required>
          </div>

          <div class="form-group">
            <label for="inp-link">{{  __('Button Link')  }}</label>
            <input type="text" class="form-control" id="inp-link" name="call_button_link"  placeholder="{{ __('Enter Link') }}" value="{{ $data->call_button_link }}" required>
          </div>
